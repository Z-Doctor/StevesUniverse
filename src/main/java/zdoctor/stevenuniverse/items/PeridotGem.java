...
public class PeridotGem extends SUItem {
	public PeridotGem() {
		super("PeridotGem");
	}
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		for (int counter = 1; counter <= 1; counter++) {
			if (playerIn.inventory.getCurrentItem().getCount() == 1) {
				playerIn.inventory.addItemStackToInventory(new ItemStack(SUItems.peridot_claw, 1));
			}
		}
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int par4, boolean par5) {
		...
		if (equipped == stack) {
			player.addPotionEffect(new PotionEffect(PotionHelper.HASTE, 6, 5));
		}
	}
}