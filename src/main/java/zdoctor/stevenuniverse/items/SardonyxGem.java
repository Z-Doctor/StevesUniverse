...
public class SardonyxGem extends SUItem {
	public SardonyxGem() {
		super("SardonyxGem");
	}
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		for (int counter = 1; counter <= 1; counter++) {
			if (playerIn.inventory.getCurrentItem().getCount() == 1) {
				playerIn.inventory.addItemStackToInventory(new ItemStack(SUItems.garnetGauntlet, 1));
				playerIn.inventory.addItemStackToInventory(new ItemStack(SUItems.pearl_spear, 1));
				playerIn.inventory.addItemStackToInventory(new ItemStack(SUItems.sardonyx_hammer, 1));
			}
		}
